...
def apply_to_collection(
    ...
    **kwargs: Any,
    ...
    if data.__class__ is list and all(
        isinstance(x, dtype) for x in data
    ):  # 1d homogeneous list
        return [function(x, *args, **kwargs) for x in data]
    if data.__class__ is tuple and all(
        isinstance(x, dtype) for x in data
    ):  # 1d homogeneous tuple
        return tuple(function(x, *args, **kwargs) for x in data)
    if data.__class__ is dict and all(
        isinstance(x, dtype) for x in data.values()
    ):  # 1d homogeneous dict
