# generation — Gemini teacher pipeline + SFT data prep
...
Moved to its own package — see [`scripts/data/relevance/README.md`](../relevance/README.md).
...
```bash
python scripts/data/generation/fix_events.py \
...
```bash
python scripts/data/generation/merge.py \
...
```bash
# Generation cost
python scripts/data/generation/costs.py dataset/zhai/v3/silver.jsonl
...
python scripts/data/generation/costs.py \
...
python scripts/data/generation/costs.py dataset/zhai/v3/silver.jsonl --batch
```
...
python scripts/data/generation/to_sft.py \
  dataset/zhai/v3/silver.final.jsonl \
  dataset/zhai/v3/silver.sft.json \
  --no-window --prompt-dir scripts/data/generation/prompts/teacher

# Strip only grounding-quote helper fields, keep document_relevance
python scripts/data/generation/to_sft.py input.jsonl output.json \
