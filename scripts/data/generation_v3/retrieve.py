...
def _embed(client: genai.Client, texts: list[str], task_type: str, model: str, batch_size: int, workers: int = 1) -> np.ndarray:
...
def main():
    ...
    parser.add_argument("--index", required=True, type=Path, help="Path to .npz label vector cache")
    ...
    if args.index.exists():
        ...
        labels, label_vectors, index_tokens = build_index(client, args.ontology, args.index, args.model, args.batch_size)
    ...
    if not args.overwrite and args.output.exists():
        ...
        if already_done:
            LOGGER.info("Resuming: skipping %d already-processed records.", already_done)
            write_mode = "a"
