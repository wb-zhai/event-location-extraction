...
Sampling is done entirely on the indexed tag tables; article content is fetched in
a final batch lookup, so the large article_downloads table is never scanned.

Usage:
    # Minimal — 50k articles, 70% positive, default seed
    python scripts/download/from_db_matrix.py \
...
#
# Two-level GROUP BY so the PK index (tag_method_id, adm_code, article_uri, ...)
# can serve a sort-merge aggregate on the innermost CTE without touching the heap.
# geo_taxonomy (small) is only joined after the first dedup, then again at country
# level — much less data than joining before the DISTINCT.
_COUNTRIES_SQL = """
WITH by_adm_code AS (
    SELECT adm_code, article_uri
    FROM article_location_tags
    WHERE tag_method_id = %(tag_method_id)s
    GROUP BY adm_code, article_uri
),
by_country AS (
...
#   2. Join to article_location_tags + geo_taxonomy to get adm0_code.
...
with_location AS (
    ...
    JOIN article_location_tags alt
        ON alt.article_uri = rs.article_uri
        AND alt.tag_method_id = %(tag_method_id)s
    JOIN geo_taxonomy geo ON alt.adm_code = geo.adm_code
...
CROSS JOIN LATERAL (
    SELECT article_uri
    FROM (
        SELECT DISTINCT alt.article_uri
        FROM article_location_tags alt
        JOIN geo_taxonomy geo ON alt.adm_code = geo.adm_code
        WHERE geo.adm0_code = c.adm0_code
          ...
          AND NOT EXISTS (
              ...
              WHERE arft.article_uri = alt.article_uri
...
def main() -> None:
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute(_COUNTRIES_SQL, {"tag_method_id": args.tag_method_id})
